...
public class NoteBookDetailDecoration extends RecyclerView.ItemDecoration {
    ...
    public NoteBookDetailDecoration(Context context, NoteBookDetailAdatper mAdapter) {
    ...
    private View getHeader(RecyclerView parent, int position) {
        int headerId = mAdapter.getHeaderId(position);
        View header = mHeaderViews.get(headerId);
        if(header == null){
            NoteBookDetailAdatper.NoteBookDetailStickyHeaderHolder headerHolder = mAdapter.createStikcyHeaderHolder(parent);
            header = headerHolder.itemView;
            mAdapter.onBindStickyHeaderViewHolder(headerHolder,position);
            //测量View并且layout
    ...
    public void onDrawOver(@NonNull Canvas c, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        ...
        if(mAdapter.getItemViewType(0) == NoteBookDetailAdatper.EMPTY_VIEW_TYPE) return;
    ...
    private int getHeaderTop(RecyclerView parent, View child, View header, int adapterPos, int layoutPos) {
        ...
        if (layoutPos == 0) {
            ...
            //Log.i("sensenote","child count:"+ count);
